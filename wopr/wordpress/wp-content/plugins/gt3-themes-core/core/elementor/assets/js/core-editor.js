	...
	$.extend(CoreEditor.prototype, {
		...
		initialize: function () {
			...
			window.elementor.channels.editor.on('section:activated', function (sectionName, editor) {
				...
				//console.log(currentElementType,sectionName,model,editor);

				if ('gt3-core-shoplist' === currentElementType && ['section_general','general_section'].includes(sectionName)) {
                    setTimeout(function () {
...
window.elementor.channels.editor.on('change:gt3-core-shoplist:prod_per_row',function(control, model){
	 model.renderHTML && model.renderHTML();
});
